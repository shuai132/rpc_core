...
simple TCP helpers provided by the JavaScript, Python, and Rust SDKs. It uses the native C++ serializer by default; enable
`RPC_CORE_SERIALIZE_USE_NLOHMANN_JSON` when cross-language JSON payload compatibility is required. For a more complete C++
networking implementation, including reconnect, SSL, UDP, serial, DDS, and discovery support, use
...
* C++ built-in adapter
    - [./cpp](./cpp): notes for C++ users. `rpc_core/net/asio_tcp.hpp` provides the built-in lightweight TCP adapter and
      requires standalone [Asio](https://think-async.com/Asio/#). It uses the native C++ serializer by default; JSON variants
      are available for cross-language testing.
...
JavaScript, Python, and Rust SDKs use JSON payloads by default. C++ uses the native binary serializer by default, including
the built-in C++ TCP adapter. For cross-language RPC, define `RPC_CORE_SERIALIZE_USE_NLOHMANN_JSON` and make
